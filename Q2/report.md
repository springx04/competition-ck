...
test 缺失规律清晰：单文本缺失 Macro-F1=0.5403，单音频=0.6099，单视觉=0.6154；rho=0.2/0.4/0.6/0.8 时分别为 0.6017/0.5902/0.5696/0.5401；front/middle/rear 为 0.5805/0.5792/0.5665。文本缺失及较长区间缺失退化最大。完整的 72 场景表、配对共同样本、实际删除率分层、8 个压力场景和图表见 [最终结果表格](docs/最终结果表格.md)。
...
还评估了 embedding 解冻、提高文本学习率、文本 stress、GRU、门控、A/V 分支、视频重复计权、回归系数、类别权重、30/40轮和多个历史 full/pool 结构；没有发现比最终单 BERT8 同时更好的可靠结果。完整 73 个保存 trial、58 条评估记录、超参数、资源与失败原因在 `results/experiment_ledger.json`，历史方法过程见 `docs/E题_Q2_优化方案_v3_实验中.md`、`docs/E题_Q2_优化实现说明_v2_实验中.md`。
...
最终包中的 `results/final/q2_predictions_aligned.csv` 有30条专项预测；专项无标签，不计算准确率。最终包 [q2_final.zip](delivery/q2_final.zip) 为 27,138,297 bytes，解压目录 30,253,681 bytes，低于 Q2 单目录 50MB；题目要求的是全题 Q1+Q2+Q3 合计不超过50MB，尚未核验全题合包大小，不能宣称整题已经满足该限制。
...
- 方法说明：[Q2最终方法与实现](docs/Q2_最终方法与实现.md)。
- 全量结果表：[最终结果表格](docs/最终结果表格.md)。
- 代码入口：`src/q2/`、`scripts/evaluate_experiment.py`、`scripts/finalize_q2.py`。
- 本地测试：当前修改相关测试已通过；服务器最终包离线重载验证通过。所有数据集、训练权重和大缓存不提交 GitHub，代码和文档位于 `Q2` 目录。
