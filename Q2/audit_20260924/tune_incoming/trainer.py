...
def train_one(root: Path, config: dict, variant: str, seed: int, resume=False, device=None):
    ...
    tune_text = variant in ("late_tune", "late_tune_balanced")
    if tune_text:
        frozen.requires_grad_(True).train()
    student = Student(variant, normalizer.class_prior, normalizer.score_prior).to(device)
    class_weight = None
    if variant in ("late_balanced", "late_tune_balanced", "late_classifier"):
    ...
    if resume and last.exists():
        ...
        if tune_text and checkpoint.get("text_encoder") is not None:
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for group in optimizer.param_groups:
            group["lr"] = learning_rate(epoch, epochs, warmup_epochs, min_lr, max_lr) if group.get("lr", 3e-4) >= 1e-4 else group["lr"]
        if tune_text:
            frozen.train()
        ...
        for batch in loader:
            ...
            descriptors = ([{"pattern": "", "rho": 0, "position": "middle"} for _ in original_indices]
                           if variant in ("late_clean", "late_balanced", "late_tune", "late_tune_balanced", "late_classifier") else
            ...
            if variant in ("late_clean", "late_balanced", "late_tune", "late_tune_balanced", "late_classifier"):
                corrupted = raw
            ...
            losses = compute_losses(clean_output, corrupt_output, teacher_output, labels, perturbation,
                                    ...
                                    regression_weight=0.25 if variant == "late_classifier" else 1.0)
        ...
        if epoch == 5 and variant not in ("late_clean", "late_balanced", "late_tune", "late_tune_balanced", "late_classifier", "late_aug", "no_teacher"):
            teacher = copy.deepcopy(student).requires_grad_(False).eval()
