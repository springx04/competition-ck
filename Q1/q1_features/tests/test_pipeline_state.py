...
from q1_features.quality import audit_pairing, vision_no_face_issue
...
def test_zero_face_result_is_an_explicit_non_text_pairing_issue() -> None:
    issue = vision_no_face_issue(
        {"successful_candidate_count": 0},
        [
            {"start": 0.0, "end": 0.04, "eligible": False, "reasons": ["no_face"]},
            {"start": 0.04, "end": 0.08, "eligible": False, "reasons": ["no_face"]},
        ],
    )
    assert issue is not None
    assert issue["issue_type"] == "vision_no_face"
    result = audit_pairing(
        diagnostic_wer=0.0,
        aligned_word_fraction=1.0,
        automatic_issues=[issue],
        reviews=[],
    )
    assert result["pairing_status"] == "suspected"
    assert result["paired_use"] is False
...
def test_no_face_issue_requires_every_row_to_be_natural_missing() -> None:
    assert vision_no_face_issue(
        {"successful_candidate_count": 0},
        [{
            "start": 0.0, "end": 0.04, "eligible": False,
