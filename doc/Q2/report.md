...
最终包中的 `results/final/q2_predictions_aligned.csv` 有30条专项预测；专项无标签，不计算准确率。最终包 `q2_final.zip`（交付包未纳入仓库） 为 27,138,297 bytes，解压目录 30,253,681 bytes，低于 Q2 单目录 50MB；题目要求的是全题 Q1+Q2+Q3 合计不超过50MB，尚未核验全题合包大小，不能宣称整题已经满足该限制。
