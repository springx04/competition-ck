...
from .quality import audit_pairing, load_reviews, vision_no_face_issue, word_error_counts
...
def stage_audio(cfg: dict[str, Any], run: Path, samples: list[Sample], *, resume: bool) -> None:
    ...
    def action(sample: Sample, sample_dir: Path) -> dict[str, Any]:
        ...
        series, names, rows = extract_audio(
            np.asarray(wav), float(media.get("audio_offset", 0.0)), smile,
            duration=float(media["duration"]),
        )
...
def stage_vision(cfg: dict[str, Any], run: Path, samples: list[Sample], *, resume: bool, reuse_raw_csv: bool = False) -> None:
    ...
    def action(sample: Sample, sample_dir: Path) -> dict[str, Any]:
        ...
        if not reuse_raw_csv:
            ...
            process_record = read_json(sample_dir / "openface" / "process.json", {})
            details_path = sample_dir / "openface" / "features_of_details.txt"
            if process_record.get("returncode") != 0 or not details_path.is_file():
                raise PipelineError("reusable OpenFace raw output is missing")
        # FaceLandmarkVidMulti exits successfully but writes no CSV when it
        # detects no face at all.  Preserve those frames as explicit no_face
        # rows instead of turning a natural missing modality into a stage
        # failure.  A failed OpenFace command is still raised by run_openface.
        candidates = read_openface_csv(csv_path) if csv_path.is_file() else []
...
def stage_audit(cfg: dict[str, Any], run: Path, samples: list[Sample]) -> None:
    ...
    for sample in samples:
        ...
        no_face = vision_no_face_issue(vision_diagnostic, vision_rows)
        if no_face is not None:
            automatic.append({"sample_id": sample.sample_id, **no_face})
